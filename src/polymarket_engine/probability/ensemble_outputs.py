...
def reduce_ensemble(
    ...
    p_finish = _weighted_mean(effective_finish, normalized)
    p_no_touch = _weighted_mean(effective_touch, normalized)
    u_gen_finish = _weighted_std(effective_finish, normalized, p_finish)
    u_gen_touch = _weighted_std(effective_touch, normalized, p_no_touch)
    ...
    return EnsembleProbability(
        ...
        uncertainty_buffer=base_model_buffer + 0.5 * u_gen + sparse_penalty,
...
def _weighted_mean(
    ...
    return sum(weights[generator_id] * value for generator_id, value in values.items())
