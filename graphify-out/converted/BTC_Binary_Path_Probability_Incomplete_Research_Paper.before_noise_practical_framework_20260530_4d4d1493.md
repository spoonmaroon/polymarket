<!-- converted from BTC_Binary_Path_Probability_Incomplete_Research_Paper.before_noise_practical_framework_20260530.docx -->
...
## 6.3 Noise Handling Inside Monte Carlo
The Monte Carlo engine should not treat every observed wiggle as equal. Bad ticks and stale sources should be filtered before simulation. Real adverse path behavior, such as wicks and danger-line touches, should remain in the path library because that is the risk being priced. The final noise estimator will be selected after reviewing the market-microstructure literature and testing simple realized volatility against noise-robust alternatives.
