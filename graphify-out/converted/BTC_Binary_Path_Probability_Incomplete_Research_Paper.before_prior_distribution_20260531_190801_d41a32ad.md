<!-- converted from BTC_Binary_Path_Probability_Incomplete_Research_Paper.before_prior_distribution_20260531_190801.docx -->
...
6.4 monte carlo test cases and unanswered questions
6.5 cached grids and refresh rules
...
The implementation details removed from the main paper are preserved in the project build plan. That plan covers portfolio sizing, exit-state machines, dashboards, kill switches, XGBoost challenger models, database schema, and operational rollout. Those components matter for a real system, but they should not distract from the core research claim: the first thing to prove is whether remaining-path probability improves binary-contract pricing and decision quality for BTC and ETH.
...
| --- | --- | --- |
| dist_to_K_bps | Distance from  to  in basis points. | Basic cushion and danger-line proximity. |
| cross_count_K_L | Number of times price crossed  in lookback window . | Detects chop around the settlement line. |
| touch_count_epsilon_L | Count of touches within  of . | Detects repeated danger-line pressure. |
| congestion_K_L | Fraction of time price spent within  of . | Flags thresholds inside noisy zones. |
