<!-- converted from BTC_Binary_Path_Probability_Incomplete_Research_Paper.before_paired_probability_sections_20260529_173753.docx -->
...
# Introduction
Short-dated BTC binary markets look simple because the payoff is only one or zero, but the pricing problem is not simply whether BTC is bullish or bearish. Each contract is tied to a specific threshold, a short expiry window, an executable market price, and a venue-defined settlement source. A useful model must therefore price the exact remaining path of the contract, not a general opinion about BTC direction.
The proposed system begins from an option-like view of the payoff. A BTC UP/DOWN market is treated as a digital payoff at settlement, while the path before settlement is treated with barrier/no-touch logic. This distinction matters because a contract can have an attractive terminal probability while still being dangerous to enter if the path is unstable, volatility is expanding, or price is too close to the threshold.
The first research objective is to determine whether a transparent remaining-path probability engine can identify moments when the executable market price is meaningfully different from fair probability after spread, fees, slippage, latency, model uncertainty, and fill risk. The engine produces p_finish, p_no_touch, z_path, fair value, edge after costs, and block reasons. These outputs are then filtered through volatility, structure, ETF options context, portfolio risk, and execution-quality checks.
This draft is intentionally incomplete. It is a research plan and methodology document, not a claim of proven edge or a recommendation to trade live capital. The near-term goal is to build a falsifiable read-only and paper-trading system, collect clean BTC binary observations, and use calibration results to decide whether the idea deserves more infrastructure.
# 1. Research Question
...
## 2.1 Settlement Source, Reference Price, and Threshold
The settlement source is the official price feed or rule that the venue uses to decide whether the binary pays out. It is the scoreboard for the contract. The model should not treat generic BTC spot, a chart price, or a random exchange last trade as truth unless the market rules say that source controls settlement.
There are three related prices that must be stored separately:
- Opening reference price / threshold K: the price captured at the start of the market window. This becomes the line that UP must finish above or DOWN must finish below.
- Current settlement-source price S_t: the latest price from the official settlement source, or the closest validated live proxy when the official tick is not directly available. This is the price used for z_path, p_finish, and p_no_touch.
- Final settlement price S_T: the official price at the resolution timestamp. This creates the final outcome label: UP pays if S_T > K; DOWN pays if S_T < K, subject to the market rule text.
Example: suppose a five-minute BTC UP/DOWN market starts at 12:00 and the official settlement-source BTC price is $100,000. Then K = $100,000. At 12:03, the current settlement-source price is $100,080, so S_t = $100,080 and the UP contract is currently on the winning side. At 12:05, the official final settlement price is $100,020, so S_T = $100,020 and UP pays one unit while DOWN pays zero.
The key point is that settlement is venue-defined. If Binance, Coinbase, Kraken, or TradingView show a slightly different BTC price, that difference may be useful as a leading indicator or sanity check, but it does not decide the payout unless the market rules name that source.
Decision rule: if the settlement source is unknown, the market is not tradable. If the settlement-source price is stale, the market is blocked. If exchange proxies disagree materially with the settlement source, the system should either widen the uncertainty buffer or block the trade until the discrepancy normalizes.
Backtest rule: every logged market must include settlement_source, threshold_price, current_settlement_price snapshots, settlement_timestamp, final_settlement_price, and the venue rule text or rule identifier. Without these fields, the model cannot honestly evaluate whether p_finish, p_no_touch, z_path, and the decision tree were correct.
...
# 4. Core Model Outputs and Calculations
...
z_path is the standardized distance between the current settlement-source BTC price and the binary threshold. It answers: how far is BTC from the line, after adjusting for how much BTC can realistically still move before expiry?
The implementation should use log distance because it matches the p_finish formula and keeps the scale consistent when BTC trades at different price levels. For small BTC moves, the log-distance version is almost the same as dollar distance divided by current price.
Variables: S_t is the current settlement-source BTC price; K is the threshold or opening reference price; sigma_remaining is the forecast remaining log-price standard deviation for the time left until expiry. sigma_remaining comes from the empirical realized-vol surface and can be widened by GARCH/HAR-RV or ETF stress warnings.
For an UP contract, favorable distance is:
distance_log_up = ln(S_t / K)
For a DOWN contract, favorable distance is:
distance_log_down = ln(K / S_t)
Then:
z_path = favorable_distance_log / sigma_remaining
...
Dollar intuition: if the UP threshold is $100,000, current settlement-source BTC is $100,080, and the expected remaining absolute move is about $40, then z_path is about 80 / 40 = 2.0. If expected remaining movement is about $160, then z_path is about 80 / 160 = 0.5. The same $80 cushion is safe in a calm window and weak in a volatile window.
...
## 4.4 First-Pass p_finish Calculation
...
## 4.6 Greeks as Binary Risk Sensitivities
...
# 5. Volatility and Risk Methodology
This section explains how the system estimates remaining movement, recognizes volatility regimes, and controls portfolio risk after the core binary outputs have been defined. The first implementation should keep this layer empirical and auditable: realized volatility first, HAR-RV/GARCH as challengers or modifiers, and portfolio risk controls before live capital.
...
## 5.2 Volatility Surface and Forecasting
...
## 5.3 Risk Layer: Correlation, VaR, and Portfolio
...
## 5.4 Deferred Models and Why They Wait
...
- The settlement-source mapping for each target Polymarket BTC market must be verified from the market rule text and logged with every observation.
...
| Settlement/reference BTC price | Venue rule text, official settlement-source ticks or validated live proxy, threshold K, current S_t snapshots, final S_T, settlement timestamp | Defines the price that actually resolves the contract. Used for z_path, p_finish, p_no_touch, stale-source flags, source-disagreement checks, and final outcome labels. |
...
| distance_log = ln(current_price / threshold_price)      # BTC UP
distance_log = ln(threshold_price / current_price)      # BTC DOWN
sigma_remaining = realized_vol_estimate * sqrt(seconds_left / seconds_per_year)
z_path = distance_log / sigma_remaining
# Dollar intuition: z_path ~= dollar_cushion / expected_dollar_move |
| --- |
| Component | Role | Decision Effect |
| --- | --- | --- |
| Realized-vol surface | Estimates expected remaining movement from historical windows matched by asset, horizon, seconds left, and regime. | Feeds sigma_remaining, z_path, p_finish, and p_no_touch. |
| HAR-RV / GARCH | Forecasts volatility clustering and short-horizon regime changes. | Widens expected move, raises uncertainty buffers, or blocks during volatility expansion. |
| Correlation / VaR / Portfolio | Controls aggregate exposure, drawdown, and capital allocation across active binaries. | Reduces size, blocks stacked exposure, and triggers kill-switches. |
| Deferred models | XGBoost later as a challenger/filter; HMM later for regime labels; deep models wait. | Improves blocking or regime classification only after out-of-sample validation. |
