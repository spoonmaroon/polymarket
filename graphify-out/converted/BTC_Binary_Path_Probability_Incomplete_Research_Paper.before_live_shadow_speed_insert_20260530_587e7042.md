<!-- converted from BTC_Binary_Path_Probability_Incomplete_Research_Paper.before_live_shadow_speed_insert_20260530.docx -->
...
The paper now follows a stricter order. First it defines the instrument and the data. Then Section 4 defines the core outputs and explains why Monte Carlo is the intended primary estimator for p_finish and p_no_touch. Section 5 defines the market-state inputs. Sections 6-8 then specify the Monte Carlo replay method, the replay dataset and labels, and the early XGBoost challenger/calibration layer. Later sections cover backtesting, ETF/GEX context, decision gates, validation, open questions, and references.
...
The default design is empirical resampling from historical BTC paths. This is preferred over a purely normal model because short-dated BTC contracts are sensitive to wicks, jumps, volatility clustering, and final-window instability.
## 6.1 Path Generation Defaults
## 6.2 Cached Grid and Refresh Rules
The live engine should not run a full Monte Carlo simulation on every tick. It should maintain cached probability grids and refresh them only when the state has changed enough to matter.
...
The replay dataset is the bridge between Monte Carlo and XGBoost. It must store every as-of decision state, every model output, every executable quote, every block reason, and every final label. Without this dataset, XGBoost has nothing honest to learn from.
## 7.1 Required Replay Rows
## 7.2 Label Definitions
## 7.3 Overfit Controls
...
- If methodology changes after holdout review, create a new later holdout before claiming success.
- Store model version, feature version, and decision rule version with every replay row.
...
- Promotion requirement: improves out-of-sample EV after costs, calibration, and drawdown without heavy opportunity deletion.
...
- How many replay rows are needed before XGBoost can be trusted as a blocker?
...
| Time step | 1-second steps when reliable data exists; 5-second fallback when 1-second data is unavailable. |
...
| Field Group | Fields |
| --- | --- |
| Contract state | contract_id, venue, side, K, expiry, seconds_left, rules, settlement_source_id |
| Market quote | best_bid, best_ask, spread, available_depth, quote_age, executable_price |
| BTC state | S_t, z_path, realized_vol windows, vol_trend, vol_regime, wick_frequency, source_quality_flag |
| Monte Carlo outputs | p_finish_MC, p_no_touch_MC, mc_uncertainty, cache_bucket_id, path_count, refresh_reason |
| Structure filters | support/resistance distance, congestion flag, recent level-cross count, block_reason |
| Outcome labels | finish_win, danger_line_touch, profitable_after_costs, false_positive, missed_winner |
| Label | Meaning |
| --- | --- |
...
| profitable_after_costs | Whether the candidate trade would have positive realized value after spread, fees, slippage, and assumptions about fill. |
...
| Ablation results | Compares core MC, MC plus structure, MC plus XGBoost, and later MC plus ETF/GEX. |